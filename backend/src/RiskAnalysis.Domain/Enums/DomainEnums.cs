...
/// <summary>
/// Классификация обнаруженной аномалии ценового ряда.
/// </summary>
public enum CorporateActionType
{
    /// <summary>
    /// Дробление акций: цена снижается кратно, стоимость позиции не меняется.
    /// Исторические котировки подлежат корректировке.
    /// </summary>
    Split = 1,

    /// <summary>
    /// Консолидация (обратное дробление): цена возрастает кратно.
    /// Исторические котировки подлежат корректировке.
    /// </summary>
    ReverseSplit = 2,

    /// <summary>
    /// Рыночное событие: резкое изменение цены отражает действительное
    /// изменение стоимости. Корректировке не подлежит и представляет
    /// ценность как исторический сценарий стресс-тестирования.
    /// </summary>
    MarketEvent = 3
}

/// <summary>Способ выявления аномалии ценового ряда.</summary>
public enum DetectionSource
{
    /// <summary>Выявлено алгоритмом предобработки данных.</summary>
    Automatic = 1,
